...
# Observacoes
- Planos empresariais implementados em 04/08/2026: nova página `/company/plans` com os planos Essencial (R$ 0/mês + R$ 99 por contratação), Profissional (R$ 199/mês + R$ 299 por contratação) e Portal VIP (R$ 299/mês + R$ 199 ou R$ 149 por contratação), comparação clara e seleção com confirmação visual. As migrations `20260804010000_add_company_plans.sql` e `20260804010100_fix_company_plan_rpc_return.sql` foram aplicadas no Supabase remoto: a escolha é registrada em `companies.plan_code`, possui histórico em `company_plan_history` e não pode ser alterada diretamente fora da RPC autorizada. A cobrança online não foi criada, pois depende da futura definição e integração do módulo financeiro.
