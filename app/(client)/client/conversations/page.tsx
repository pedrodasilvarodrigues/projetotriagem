...
export default async function ClientConversationsPage() {
  ...
  const { data } = client ? await supabase.from("marketplace_conversations").select("id,status,last_message_at,created_at,service_provider_profiles(professional_title)").eq("client_id", client.id).order("updated_at", { ascending: false }) : { data: [] };
