...
export default async function CompanyNotificationsPage() {
  ...
  return (
    <AppShell eyebrow="Empresa" title="Notificações">
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="space-y-3">
          {(notifications ?? []).map((item) => <article key={item.id} className="rounded-md border border-slate-200 bg-slate-50 p-4"><h2 className="font-semibold">{item.title}</h2><p className="mt-2 text-sm text-slate-600">{item.body}</p><p className="mt-2 text-xs text-slate-500">{item.read_at ? "Lida" : "Não lida"}</p></article>)}
