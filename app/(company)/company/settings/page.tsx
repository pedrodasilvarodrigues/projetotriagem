...
import { requestTypeLabel, statusLabel } from "@/lib/status-labels";
...
export default async function CompanySettingsPage({ searchParams }: { searchParams: Promise<{ error?: string; message?: string }> }) {
  ...
  return (
    <AppShell eyebrow="Empresa" title="Configurações">
        ...
        <aside className="space-y-5">
          ...
          <form action={requestDataExportAction} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <select name="requestType" className="field-input mt-5">
              ...
              <option value="account_deletion">Solicitar exclusão</option>
            </select>
        ...
        <div className="space-y-5">
          <form action={updateUserSettingsAction} className="border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <SettingsPreferences
              ...
              labels={{
                ...
                profileVisibleTitle: "Empresa visível internamente",
                ...
                salaryDescription: "Exibir informações complementares quando a demanda utilizar faixas e preferências."
              }}
            />
            ...
            <section className="mt-5 border border-slate-200 bg-slate-50 p-4">
              ...
              <p className="mt-3 text-sm leading-6 text-slate-600">
                Essas configurações controlam como a plataforma envia avisos, apresenta seu ambiente e aplica a preferência de idioma.
              </p>
            </section>
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <table className="data-table mt-4">
              <thead><tr><th>Tipo</th><th>Situação</th><th>Data</th></tr></thead>
              <tbody>
                {(requests ?? []).map((request) => <tr key={request.id}><td>{requestTypeLabel(request.request_type)}</td><td>{statusLabel(request.status)}</td><td>{new Date(request.created_at).toLocaleDateString("pt-BR")}</td></tr>)}
