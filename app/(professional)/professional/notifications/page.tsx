...
export default async function ProfessionalNotificationsPage() {
  ...
  const { data: marketplaceNotifications } = marketplaceEnabled
    ? await supabase.from("marketplace_notifications").select("id,title,body,read_at,created_at,link_path").eq("user_id", userData.user?.id).order("created_at", { ascending: false })
    : { data: [] };
  ...
  return (
    <AppShell eyebrow="Profissional" title="Notificações">
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        ...
        {marketplaceEnabled ? <form action={markMarketplaceNotificationsReadAction} className="mb-4 flex justify-end"><button className="rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold" type="submit">Marcar mensagens de serviços como lidas</button></form> : null}
