...
export default async function MarketplaceConversationPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  return <AppShell eyebrow={eyebrow} title="Negociação de serviço">
    ...
    {query.success && <p className="mb-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-700">Operação concluída.</p>}
    <div className="grid gap-6 xl:grid-cols-[1fr_390px]">
      ...
      <aside className="space-y-5">
        ...
        {isRequester && !alreadyReviewed ? <section className="rounded-2xl border border-amber-200 bg-white p-5"><h2 className="font-bold text-[#0F2D4E]">Avaliar prestador</h2><p className="mt-1 text-xs leading-5 text-slate-500">Disponível após a conversa ser criada. Publique somente uma experiência real.</p><form action={createServiceReviewAction} className="mt-4"><input type="hidden" name="conversationId" value={id} /><label className="text-xs font-bold">Nota<select name="rating" className={input} required><option value="5">5 — Excelente</option><option value="4">4 — Muito bom</option><option value="3">3 — Bom</option><option value="2">2 — Ruim</option><option value="1">1 — Péssimo</option></select></label><textarea name="comment" className={input} required minLength={3} placeholder="Conte como foi sua experiência" /><button className="mt-3 w-full rounded-lg bg-[#0F2D4E] px-3 py-2.5 text-xs font-bold text-white">Publicar avaliação</button></form></section> : null}
