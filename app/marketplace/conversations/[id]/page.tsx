...
export default async function MarketplaceConversationPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  const [{ data: conversation }, { data: messages }, { data: requests }] = await Promise.all([
    supabase.from("marketplace_conversations").select("id,status,provider_id,client_id").eq("id", id).maybeSingle(),
  ...
  const { data: reviews } = requestIds.length ? await supabase.from("service_reviews").select("request_id").in("request_id", requestIds) : { data: [] };
  const reviewed = new Set(reviews?.map((item) => item.request_id));
  return <AppShell eyebrow={eyebrow} title="Negociação de serviço">
    {query.error && <p className="mb-4 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">{decodeURIComponent(query.error)}</p>}
    <div className="grid gap-6 xl:grid-cols-[1fr_390px]"><ChatPanel conversationId={id} currentUserId={userData.user.id} initialMessages={messages ?? []} />
      <aside className="space-y-5">
        {role === "client" && conversation.status === "open" && <details className="rounded-2xl border border-slate-200 bg-white p-5" open={!requests?.length}><summary className="flex cursor-pointer items-center gap-2 font-bold text-[#0F2D4E]"><ClipboardPlus size={19} />Criar solicitação formal</summary><form action={createServiceRequestAction} className="mt-4 space-y-3"><input type="hidden" name="conversationId" value={id} /><label className="text-sm font-semibold">Serviço<input name="title" required minLength={3} className={input} /></label><label className="text-sm font-semibold">Descrição<textarea name="description" required minLength={10} rows={3} className={input} /></label><label className="text-sm font-semibold">Forma de preço<select name="pricingModel" className={input}><option value="quote">Orçamento</option><option value="negotiable">A combinar</option><option value="fixed">Valor fixo</option><option value="hourly">Por hora</option></select></label><label className="text-sm font-semibold">Valor proposto opcional<input name="amount" type="number" min="0" step="0.01" className={input} /></label><button className="w-full rounded-xl bg-[#F2811D] px-4 py-3 text-sm font-bold text-white">Enviar solicitação</button></form></details>}
        <section className="rounded-2xl border border-slate-200 bg-white p-5"><h2 className="font-bold text-[#0F2D4E]">Solicitações</h2><div className="mt-4 space-y-4">{requests?.length ? requests.map((request) => <article key={request.id} className="rounded-xl border border-slate-200 p-4"><div className="flex justify-between gap-2"><strong className="text-sm text-[#0F2D4E]">{request.title}</strong><span className="rounded-full bg-blue-50 px-2 py-1 text-[10px] font-bold text-[#0F2D4E]">{labels[request.status] ?? request.status}</span></div><p className="mt-2 text-xs leading-5 text-slate-600">{request.description}</p>{request.proposed_amount && <p className="mt-2 text-sm font-bold">R$ {Number(request.proposed_amount).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</p>}
          <div className="mt-3 flex flex-wrap gap-2">{role === "professional" && request.status === "awaiting_response" && <><MiniAction requestId={request.id} conversationId={id} status="accepted" label="Aceitar" /><MiniAction requestId={request.id} conversationId={id} status="rejected" label="Recusar" danger /></>}{request.status === "accepted" && <MiniAction requestId={request.id} conversationId={id} status="in_progress" label="Iniciar serviço" />}{["accepted","in_progress"].includes(request.status) && <form action={confirmServiceCompletionAction}><input type="hidden" name="conversationId" value={id} /><input type="hidden" name="requestId" value={request.id} /><button className="rounded-lg bg-emerald-700 px-3 py-2 text-xs font-bold text-white">Confirmar conclusão</button></form>}{!["completed","cancelled","rejected"].includes(request.status) && <MiniAction requestId={request.id} conversationId={id} status="cancelled" label="Cancelar" danger />}</div>
          {role === "client" && request.status === "completed" && !reviewed.has(request.id) && <form action={createServiceReviewAction} className="mt-4 rounded-xl bg-slate-50 p-3"><input type="hidden" name="conversationId" value={id} /><input type="hidden" name="requestId" value={request.id} /><label className="text-xs font-bold">Nota<select name="rating" className={input} required><option value="5">5 — Excelente</option><option value="4">4 — Muito bom</option><option value="3">3 — Bom</option><option value="2">2 — Ruim</option><option value="1">1 — Péssimo</option></select></label><textarea name="comment" className={input} placeholder="Conte como foi o serviço" /><button className="mt-2 w-full rounded-lg bg-[#0F2D4E] px-3 py-2 text-xs font-bold text-white">Publicar avaliação</button></form>}
        </article>) : <p className="text-sm text-slate-500">Nenhuma solicitação formal.</p>}</div></section>
