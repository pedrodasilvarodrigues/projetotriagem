...
export default async function ProfessionalResumePage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return (
    <OnboardingLayout title="Curriculo profissional" description="Envie seu curriculo atual ou preencha os dados para gerar uma versao profissional automaticamente.">
      <ErrorBanner error={params.error} />
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="text-lg font-semibold">Sim, possuo curriculo</h2>
          ...
          <form action={uploadResumeAction} className="mt-5 space-y-4">
            <label className="block rounded-lg border border-dashed border-slate-300 bg-slate-50 p-6 text-center text-sm text-slate-600">
              Selecione o arquivo do curriculo
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="text-lg font-semibold">Nao possuo curriculo</h2>
          ...
          <form action={generateResumeAction} className="mt-5 space-y-6">
            <fieldset className="grid gap-4 md:grid-cols-2">
              <legend className="mb-3 text-sm font-semibold text-slate-900 md:col-span-2">Formacao</legend>
