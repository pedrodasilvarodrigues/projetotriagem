  ...
  return (
    <form action={registerCompanyWithEmailAction} onSubmit={() => setSubmitting(true)} className="space-y-5">
      ...
      <fieldset className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        <label className="block text-sm font-medium text-slate-800">Razão Social<input name="legalName" required value={legalName} onChange={(event) => setLegalName(event.target.value)} className="mt-1 w-full rounded-md border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-950 transition focus:border-blue-700" /></label>
        ...
        <label className="block text-sm font-medium text-slate-800 lg:col-span-2">Endereço<input name="street" required value={street} onChange={(event) => setStreet(event.target.value)} className="mt-1 w-full rounded-md border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-950 transition focus:border-blue-700" /></label>
        <label className="block text-sm font-medium text-slate-800">Número<input name="addressNumber" required value={number} onChange={(event) => setNumber(event.target.value)} className="mt-1 w-full rounded-md border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-950 transition focus:border-blue-700" /></label>
      ...
      <fieldset className="grid gap-4 md:grid-cols-3">
        <legend className="mb-1 text-sm font-semibold text-slate-950 md:col-span-3">Responsável</legend>
